...
训练: 8个受试者的 *T.gdf
测试: 留一受试者的 *E.gdf (使用官方标签)
完全按照 BCI Competition 评估协议
"""
...
class BCIC2aTestDataset(Dataset):
    ...
    def __init__(self, data_dir, labels_dir, subject_id):
        ...
        # 尝试找到标签（常见格式）
        if 'classlabel' in label_data:
            y = label_data['classlabel'].flatten().astype(np.int64) - 1  # 1-4 → 0-3
        elif 'labels' in label_data:
            y = label_data['labels'].flatten().astype(np.int64) - 1
        elif 'event' in label_data:
            # 从 event 结构中提取
            event = label_data['event'][0, 0]
            y = event['labels'][0, 0].flatten().astype(np.int64)
        else:
            raise ValueError(f"未知标签格式，可用keys: {label_data.keys()}")
        
        # 获取事件时间点，提取对应epochs
        events, event_dict = mne.events_from_annotations(raw, verbose='ERROR')
        picks = mne.pick_types(raw.info, meg=False, eeg=True, exclude='bads')
        epochs = mne.Epochs(raw, events, tmin=0.5, tmax=2.5, picks=picks,
                            baseline=None, preload=True, verbose='ERROR')
        ...
        self.X = X.astype(np.float32)
        self.y = y.astype(np.int64)
        print(f"Test subject {subject_id}: {len(self.y)} samples, labels: {np.bincount(self.y)}")
