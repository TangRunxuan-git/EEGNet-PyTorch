...
class BCIC2aTestDataset(Dataset):
    ...
    def __init__(self, data_dir, labels_dir, subject_id):
        ...
        # *E.gdf 中只有 768 事件 (trial 开始标记)
        # 找到事件代码为 768 对应的 ID (从输出看是 6)
        target_code = None
        ...
        print(f"找到 {len(cue_events)} 个 trial 起始事件")
        ...
        for event in cue_events:
            onset = event[0] / sfreq  # 转换为秒
            start_sample = int((onset + 0.5) * sfreq)
            end_sample = int((onset + 2.5) * sfreq)
            
            if end_sample <= raw.n_times:
                data = raw.get_data(picks=picks, start=start_sample, stop=end_sample)
                if data.shape[1] == int(2.0 * sfreq):  # 2秒窗口
                    X_list.append(data)
        ...
        # 确保数据与标签数量匹配
        if len(X) != len(self.y):
            print(f"警告: 提取的 trials={len(X)}, 标签={len(self.y)}，截断")
